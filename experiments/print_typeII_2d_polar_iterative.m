...
for it_beta = 1 : num_beta
    ...
    fprintf("\\bottomrule\n")
    fprintf("\\end{tabular}\n")
    fprintf("\\caption{Time cost, iteration number, relative residual and relative error of LSQR and PLSQR for 2D type-II NUDFT with an ill-conditioned polar grid. \\(M \\approx %.2f N \\log_{4} N\\).}\n", beta_av)
    fprintf("\\label{tab:typeII_2d_polar_bad_iterative_%.1f}\n", beta)
    fprintf("\\end{table}\n")
    fprintf("\n\n")
end
