...
for it_alpha = 1 : num_alpha
    ...
    fprintf("\\toprule\n")
    fprintf("\\(N\\) & \\(\\varepsilon\\) & \\(t_{\\mathrm{c}}\\) & \\(t_{\\mathrm{f}}\\) & \\(t_{\\mathrm{s}}\\) & \\(r_{\\mathrm{s}}\\) & \\(e_{\\mathrm{s}}\\) \\\\ \n")
    for it_n = 1 : num_n
        fprintf("\\midrule\n")
        n = n_list(it_n);
        tmp_str = string(n) + "^2";
    ...
    fprintf("\\end{tabular}\n")
    if alpha == 1.5
        fprintf("\\caption{Construction time, factorization time, solve time, relative residual and relative error of the direct solver on a random grid. \\(M = 1.5 N\\).}\n")
        fprintf("\\label{tab:typeII_2d_rand_direct_1.5}\n")
    elseif alpha == 2.0
        fprintf("\\caption{Construction time, factorization time, solve time, relative residual and relative error of the direct solver on a random grid. \\(M = 2 N\\).}\n")
