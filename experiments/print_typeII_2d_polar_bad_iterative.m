...
for it_n = 1 : num_n
    ...
    load("./typeII_2d_polar_lsqr_bad/data/typeII_2d_results_" + string(p) + ".mat");
    ...
    for it_tol = 1 : num_tol
        ...
        load("./typeII_2d_polar_plsqr_bad/data/typeII_2d_results_" + string(p) + "_tol_" + string(tol_hss) + ".mat");
...
fprintf("\\bottomrule\n")
fprintf("\\end{tabular}\n")
fprintf("\\caption{Time cost, iteration number, relative residual and relative error of LSQR and PLSQR for 2D type-II NUDFT with an ill-conditioned polar grid. \\(M \\approx 4 N\\).}\n")
fprintf("\\label{tab:typeII_2d_polar_bad_iterative}\n")
fprintf("\\end{table}\n")
fprintf("\n\n")
